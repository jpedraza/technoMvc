...
class PDF extends FPDF
{
	...
	function Header()
	{
		 ...
		 */
		$this->Cell(0,10,'Listado de Usuarios al '.date('d/m/Y'),0,0,'C');
	...
	function FancyTable($header, $data){
		...
		while ($row = $consulta->fetch_row()) {
	        ...
	        $this->Cell($w[2],8,substr(utf8_decode($row[2]),0,39),'LR',0,'L',$fill);
	        $this->Cell($w[3],8,substr(utf8_decode($row[3]),0,39),'LR',0,'L',$fill);
	        if (isset($row[4]) && $row[4]==2) {
				$permiso = 'Administrador';
			} else if(isset($row[4]) && $row[4] == 1) {
				$permiso = 'Cliente';
			} else{
				$permiso = 'Cliente Sancionado';
			}
	        $this->Cell($w[4],8,$permiso,'LR',0,'C',$fill);
	        $activo = ($row[5] == 1) ? "Si" : "No";
	        $this->Cell($w[5],8,$activo,'LR',0,'C',$fill);
	        $this->Cell($w[6],8,$row[6],'LR',0,'C',$fill);
	        $this->Ln();
	        $fill = !$fill;
	    }
	    // Línea de cierre
	    ...
	    //Consulta para obtener total usuarios.
	    $sql 	= $db->query("
	    	SELECT 
	    		COUNT(user),
	    		SUM(activo)
	    ...
	    $this->Cell(0,20,'Total de Usuarios: '.number_format($total[0],0,",","."),0,0,'L');
	    $this->Ln(5);
	    $this->Cell(0,20,'Usuarios  Activos: '.number_format($total[1],0,",","."),0,0,'L');
	    $this->Ln(5);
	    $sql 	= $db->query("
	    	SELECT 
	    		COUNT(permisos)
	    	...
	    	WHERE
	    		permisos = 2;");
	    $permisos  = $db->recorrer($sql)[0];
	    $this->Cell(0,20,'Administradores: '.number_format($permisos,0,",","."),0,0,'L');
	    $this->Ln(5);
	    $sql 	= $db->query("
	    	SELECT 
	    		COUNT(permisos)
	    	...
	    	WHERE
	    		permisos = 1;");
	    $permisos  = $db->recorrer($sql)[0];
	    $this->Cell(0,20,'Clientes: '.number_format($permisos,0,",","."),0,0,'L');
	    $this->Ln(5);
	    $sql 	= $db->query("
	    	SELECT 
	    		COUNT(permisos)
	    	...
	    	WHERE
	    		permisos = 0;");
	    $permisos  = $db->recorrer($sql)[0];
	    $this->Cell(0,20,'Clientes Sancionados: '.number_format($permisos,0,",","."),0,0,'L');
	    $db->liberar($sql);
	    $db->close();
	}
}
...
$header = array(utf8_decode('Id'), 'Nombre', utf8_decode('Usuario'), utf8_decode('Email'), utf8_decode('Permisos'), utf8_decode('¿Activo?'), utf8_decode('Ultima Visita'));
 ...
 */
$pdf->Output('D','Reporte_Usuarios_al_'.date('d/m/Y').'.pdf',true);
?>