...
if((isset($_POST['titulo']) && $_POST['titulo'] != "")  && (isset($_POST['detalle_promo']) && $_POST['detalle_promo'] != "") && (isset($_POST['oferta']) && $_POST['oferta'] != "" && ($_POST['oferta'] == 1 || $_POST['oferta'] == 0))) {
     ...
     */
    $sql = $db->query("SELECT id FROM promociones WHERE titulo='$titulo' LIMIT 1;");    
    if($db->rows($sql) == 0) {
                            ...
                            if (is_uploaded_file($_FILES['imagen']['tmp_name'])) {
                                ...
                                if ($db->rows($sql) == 0) {
                                    ...
                                    if (move_uploaded_file($nombre_tmp, "views/images/home/promociones/$nombre")){
                                        ...
                                        $sql = $db->query(
                                            "INSERT INTO
                                                promociones (titulo,detalle,imagen,oferta)
                                            VALUES
                                                ('$titulo','$detalle_promo','$nombre','$oferta')
                                            ;"
                                        );      
                                        echo  
                                        '<div class="alert alert-dismissible alert-success">
                                            <button type="button" class="close" data-dismiss="alert">x</button>
                                            <strong>Completado!:</strong> La promoción ha sido agregada exitosamente
                                        </div>';
                                    } else {
                                        echo 
