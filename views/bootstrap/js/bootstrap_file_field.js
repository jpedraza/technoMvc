        ...
        this.filter( ":file" ).each(function() {
            ...
            $(this).on('change', function(e) {
                ...
                if(maxNumFiles && this.files.length > maxNumFiles) {
                    $(this).trigger('fileSelectionError', [maxNumFiles +' Imagenes permitidas solamente!', fileList]);
                    return abort(this);
                }

                // Check min file number
                if(minNumFiles && this.files.length < minNumFiles) {
                    $(this).trigger('fileSelectionError', ['Tienes que seleccionar al menos '+ minNumFiles +' archivo!', fileList]);
                    return abort(this);
                }
                ...
                if(maxTotalSize) {
                    ...
                    if(totalSize > maxTotalSize) {
                        $(this).trigger('fileSelectionError', ['El tamaño total de los archivos seleccionados no debe exceder '+ maxTotalSize +' byte!', fileList]);
                        return abort(this);
                    }
                }
                ...
                for (var x = 0; x < this.files.length; x++) {
                    ...
                    if(fileTypes && fileTypes.indexOf(file.type) < 0) {
                        $(this).trigger('fileSelectionError', [file.name +' : El tipo de archivo no es valido! (JPG / PNG / GIF) ', fileList]);
                        return abort(this);
                    }

                    // Check max size
                    if(maxFileSize && file.size > maxFileSize) {
                        $(this).trigger('fileSelectionError', [file.name +' : Excede el tamaño maximo permitido de ' +maxFileSize/1000 + 'Kb', fileList]);
                        return abort(this);
                    }

                    // Check min size
                    if(minFileSize && file.size < minFileSize) {
                        $(this).trigger('fileSelectionError', [file.name +' : Menor tamaño de archivo permitido!', fileList]);
                        return abort(this);
                    }
