...
var nicEditorConfig = bkClass.extend({
    buttons: {
        ...
        'left': {name: __('Alinear Izquierda'), command: 'justifyleft', noActive: true},
        ...
        'ol': {name: __('Lista Ordenada'), command: 'insertorderedlist', tags: ['OL']},
        'ul': {name: __('Insertar Viñeta'), command: 'insertunorderedlist', tags: ['UL']},
        'subscript': {name: __('Click to Subscript'), command: 'subscript', tags: ['SUB']},
        'superscript': {name: __('Click to Superscript'), command: 'superscript', tags: ['SUP']},
        'strikethrough': {name: __('Click to Strike Through'), command: 'strikeThrough', css: {'text-decoration': 'line-through'}},
        'removeformat': {name: __('Remove Formatting'), command: 'removeformat', noActive: true},
        'indent': {name: __('Agregar Sangria'), command: 'indent', noActive: true},
        'outdent': {name: __('Remover Sangria'), command: 'outdent', noActive: true},
...
var nicEditor = bkClass.extend({
    construct: function (C) {
        ...
        var A = new bkElement("DIV").setStyle({width: (parseInt(B.getStyle("width")) || B.clientWidth) + "%"}).appendBefore(B);
        this.setPanel(A);
        ...
        return{
            backgroundImage: "url('" + ((C) ? this.options.iconsPath : B) + "')", backgroundPosition: ((C) ? ((C - 1) * -18) : 0) + "px 0px"
        }
...
var nicEditorInstance = bkClass.extend({
    isSelected: false, construct: function (G, D, C) {
        ...
        if (H || this.options.hasPanel) {
            ...
            var E = {width: newX + "%", border: "1px solid #ccc", borderTop: 0, overflowY: "hidden", overflowX: "auto"};
            ...
            var A = new bkElement("DIV").setStyle({width: (newX - 8) + "%", margin: "4px", minHeight: (newY + 125) + "px"}).addClass("main").appendTo(this.editorContain);
...
var nicEditorIFrameInstance = nicEditorInstance.extend({savedStyles: [], init: function () {
        ...
        this.elmFrame = new bkElement("iframe").setAttributes({src: "javascript:;", frameBorder: 0, allowTransparency: "true", scrolling: "no"}).setStyle({height: "100px", width: "10%"}).addClass("frame").appendTo(this.elm);
        if (this.copyElm) {
...
/*
 * PANEL DESPLEGADO AL PULSAR UN BOTON
*/
...
/*
 * PANEL DE LETRAS TAMAÑOS Y TIPOS
 */ 
...
/*
 * TEXTO QUE APARECE AL POSICIONAR EL BOTON SOBRE LOS ICONOS (ALT)
 */ 
var nicButtonTips = bkClass.extend({construct: function (A) {
        ...
        this.timer = setTimeout(this.create.closure(this, A), 600)
    }, create: function (A) {
        this.timer = null;
        if (!this.pane) {
...
/*
 * PALETA DE COLORES QUE APARECE AL PRESIONAR EL BOTON
 */ 
...
var nicLinkOptions = {
    buttons: {
        'link': {name: 'Agregar Link', type: 'nicLinkButton', tags: ['A']},
...
var nicLinkButton = nicEditorAdvancedButton.extend({addPane: function () {
        ...
        this.addForm({"": {type: "title", txt: "Agregar/Editar Link"}, href: {type: "text", txt: "URL", value: "http://", style: {width: "150px"}}, title: {type: "text", txt: "Title"}, target: {type: "select", txt: "Open In", options: {"": "Current Window", _blank: "New Window"}, style: {width: "100px"}}}, this.ln)
    }, submit: function (C) {
        var A = this.inputs.href.value;
        if (A == "http://" || A == "") {
            alert("Debe introducir una URL para crear un enlace");
            return false
        }
        this.removePane();
        if (!this.ln) {
...
var nicColorOptions = {
    buttons: {
        'forecolor': {name: __('Cambiar Color del Texto'), type: 'nicEditorColorButton', noClose: true},
        'bgcolor': {name: __('Cambiar Color del Fondo'), type: 'nicEditorBgColorButton', noClose: true}
    }
};

/*
 * TAMAÑO DE COLORES DE LA PALETA
 */ 
...
var nicImageOptions = {
    buttons: {
        'image': {name: 'Agregar Imagen', type: 'nicImageButton', tags: ['IMG']}
    }
...
var nicImageButton = nicEditorAdvancedButton.extend({
    addPane: function () {
        ...
        this.addForm({"": {type: "title", txt: "Agregar/Editar Imagen"},
            src: {type: "text", txt: "URL", value: "", style: {width: "75%"}},
            alt: {type: "text", txt: "Titulo", style: {width: "75%"}},
            width: {type: "text", txt: "Ancho", style: {width: "32%"}},
            height: {type: "text", txt: "Alto", style: {width: "32%"}},
            align: {type: "select", txt: "Alinear", options: {none: "Defecto", left: "Izquierda", right: "Derecha"}}},
        this.im)
    }, submit: function (B) {
        var C = this.inputs.src.value;
        if (C == "") {
            alert("Debe introducir una URL para insertar la imagen");
            return false
        }
        this.removePane();
        if (!this.im) {
        ...
        if (this.im) {
            this.im.setAttributes({
                src: this.inputs.src.value, alt: this.inputs.alt.value, width:this.inputs.width.value, height:this.inputs.height.value, align: this.inputs.align.value, 
            })
        }
    }
});
...
var nicSaveOptions = {
    buttons: {
        'save': {name: __('Guardar este Contenido'), type: 'nicEditorSaveButton'}
    }
};
...
var nicUploadOptions = {
    buttons: {
        'upload': {name: 'Cargar Imagen', type: 'nicUploadButton'}
    }
...
var nicUploadButton = nicEditorAdvancedButton.extend({
    nicURI: "https://api.imgur.com/3/image", errorText: "Failed to upload image", addPane: function () {
        ...
        new bkElement("div").setStyle({fontSize: "14px", fontWeight: "bold", paddingBottom: "5px"}).setContent("Inserta una Imagen").appendTo(A);
        ...
        this.removePane();
        alert(A || "Error al cargar la Imagen")
    }, uploadFile: function () {
        var B = this.fileInput.files[0];
        if (!B || !B.type.match(/image.*/)) {
            this.onError("Solo puede cargar Imagenes");
            return
        }
